...
{
    /// <summary>
    /// A custom shader inspector for the "Graphics Tools/Standard" shader.
    /// </summary>
    public class MixedRealityStandardShaderGUI : MixedRealityShaderGUI
    {
        ...
        [MenuItem("Window/Graphics Tools/Upgrade Graphics Tools shaders for Universal Render Pipeline")]
#else
        [MenuItem("Window/Graphics Tools/Upgrade Graphics Tools shaders for Lightweight Render Pipeline")]
#endif
        protected static void UpgradeShaderForUniversalRenderPipeline()
        {
            string confirmationMessage;
#if UNITY_2019_1_OR_NEWER
            confirmationMessage = "This will alter shaders for use with Unity's Universal Render Pipeline. You cannot undo this action.";
#else
            confirmationMessage = "This will alter shaders for use with Unity's Lightweight Render Pipeline. You cannot undo this action.";
#endif

            if (EditorUtility.DisplayDialog("Upgrade shaders?",
                                            confirmationMessage,
                                            "Ok",
                                            "Cancel"))
            {
                string path = AssetDatabase.GetAssetPath(StandardShaderUtility.GraphicsToolsStandardShader);
                ...
                {
                    Debug.LogErrorFormat("Failed to get asset path to: {0}", StandardShaderUtility.GraphicsToolsStandardShaderName);
                }
            }
        }

#if UNITY_2019_1_OR_NEWER
        [MenuItem("Window/Graphics Tools/Upgrade Graphics Tools shaders for Universal Render Pipeline", true)]
#else
        [MenuItem("Window/Graphics Tools/Upgrade Graphics Tools shaders for Lightweight Render Pipeline", true)]
#endif
