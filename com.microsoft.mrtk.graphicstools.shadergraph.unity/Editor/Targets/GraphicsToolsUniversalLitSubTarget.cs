...
{
    sealed class GraphicsToolsUniversalLitSubTarget : UniversalSubTarget, ILegacyTarget
    {
        static readonly GUID kSourceCodeGuid = new GUID("0198389c297c81342bd3d8de56fc6c58"); // GraphicsToolsUniversalLitSubTarget.cs
        ...
        public override void Setup(ref TargetSetupContext context)
        {
            ...
            // Process SubShaders
            context.AddSubShader(PostProcessSubShader(SubShaders.LitSubShader(target, workflowMode, target.renderType, target.renderQueue, target.disableBatching, complexLit, blendModePreserveSpecular, overrideBlendAlpha, srcBlendAlpha, dstBlendAlpha)));
        }
        ...
        {
            public static SubShaderDescriptor LitSubShader(UniversalTarget target, WorkflowMode workflowMode, string renderType, string renderQueue, string disableBatchingTag, bool complexLit, bool blendModePreserveSpecular, bool overrideBlendAlpha, Blend alphaSrc, Blend alphaDst)
            {
                ...
                if (complexLit)
                    result.passes.Add(LitPasses.ForwardOnly(target, workflowMode, complexLit, blendModePreserveSpecular, overrideBlendAlpha, alphaSrc, alphaDst, CoreBlockMasks.Vertex, LitBlockMasks.FragmentComplexLit, CorePragmas.Forward, LitKeywords.Forward));
                else
                    result.passes.Add(LitPasses.Forward(target, workflowMode, blendModePreserveSpecular, overrideBlendAlpha, alphaSrc, alphaDst, CorePragmas.Forward, LitKeywords.Forward));

                if (!complexLit)
                    result.passes.Add(LitPasses.GBuffer(target, workflowMode, blendModePreserveSpecular, overrideBlendAlpha, alphaSrc, alphaDst));
                ...
                result.passes.Add(PassVariant(LitPasses._2D(target, overrideBlendAlpha, alphaSrc, alphaDst), CorePragmas.Default));
